...
if not IsAddOnLoaded("Deadly Boss Mods") then return end
...
local fontsize = 8
local fontoutline = "OUTLINEMONOCHROME"
	...
	local function SkinBars(self)
		for bar in self:GetBarIterator() do
			if not bar.injected then
					...
					tbar:SetStatusBarTexture(texture)
					...
					if not timer.styled then	
						...
						timer:SetFont(font, fontsize, fontoutline)
		...
		if not anchor.styled then
			local header={anchor:GetRegions()}
				if header[1]:IsObjectType("FontString") then
					header[1]:SetFont(font, fontsize, fontoutline)
...
local Init = function()
	...
	DBT_SavedOptions["DBM"].Texture = C.media.Glamour
	DBT_SavedOptions["DBM"].Font = C.media.pixelfont
end
