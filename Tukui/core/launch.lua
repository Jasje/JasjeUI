...
local v = CreateFrame("Button", "TukuiVersionFrame", UIParent)
    ...
    v.Text:SetText(hexa.."Jasje UI - Version "..hexb..T.version..hexa..", by jasje@tukui.org,"..hexb.." website at "..hexa.."www.tukui.org")
    v:SetScript("OnClick", function()
	    v:Hide()
    end)
    v:Hide()

    v.bg = CreateFrame("Frame", "Jasjeicon", v)
    v.bg:CreatePanel(nil, 50, 50, "BOTTOMRIGHT", v, "LEFT", -3, -12)
    v.bg:EnableMouse(true)

    v.tex = v.bg:CreateTexture(nil, "ARTWORK")
    v.tex:Point("TOPLEFT", 2, -2)
    v.tex:Point("BOTTOMRIGHT", -2, 2)
    v.tex:SetTexture(C.media.jasjeicon)
...
local sb = CreateFrame("StatusBar", nil, f)
    ...
    sb:Hide()
--
local j = CreateFrame("Button", "TukuiVersionFrameTut", f)
    j:SetSize(340, 25)
    j:SetPoint("BOTTOMRIGHT", f, "TOPRIGHT", 0, 5)
    j:SetTemplate("Transparent")
    j:SetBorder()
    j:FontString("Text", C.media.font, 12)
    j.Text:SetPoint("CENTER")
    j.Text:SetText(hexa.."Jasje UI - Version "..hexb..T.version..hexa..", by jasje@tukui.org,"..hexb.." website at "..hexa.."www.tukui.org")
    v:SetScript("OnClick", function()
	    v:Hide()
    end)
    v:Hide()

    j.bg = CreateFrame("Frame", "Jasjeicon", f)
    j.bg:CreatePanel(nil, 50, 50, "BOTTOMLEFT", f, "TOPLEFT", 0, 5)
    j.bg:SetFrameStrata("HIGH")

    j.tex = j.bg:CreateTexture(nil, "ARTWORK")
    j.tex:Point("TOPLEFT", 2, -2)
    j.tex:Point("BOTTOMRIGHT", -2, 2)
    j.tex:SetTexture(C.media.jasjeicon)
--
...
StaticPopup1:Point("CENTER", UIParent, 0, -200)
